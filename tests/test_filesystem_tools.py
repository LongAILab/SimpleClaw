...
from simpleclaw.agent.tools.filesystem import (
    EditFileTool,
    ListDirTool,
    ReadFileTool,
    WriteFileTool,
...
class _DummyDocumentStore:
    ...
    def get_active_content(self, tenant_key: str, doc_type: str, doc_name: str):
        return self.docs.get((tenant_key, doc_type, doc_name))

    def upsert_document(self, *, tenant_key: str, doc_type: str, doc_name: str, content: str, **kwargs):
        self.docs[(tenant_key, doc_type, doc_name)] = content
        return 1


class _DummyTenantStateRepo:
    def __init__(self):
        self.calls = []

    def configure_heartbeat(self, tenant_key: str, **kwargs):
        self.calls.append((tenant_key, kwargs))


class TestWriteFileTool:

    @pytest.mark.asyncio
    async def test_heartbeat_write_updates_tenant_interval(self, tmp_path):
        document_store = _DummyDocumentStore()
        tenant_state_repo = _DummyTenantStateRepo()
        tool = WriteFileTool(
            workspace=tmp_path,
            document_store=document_store,
            tenant_state_repo=tenant_state_repo,
        )
        tool.set_context(tmp_path, None, tenant_key="tenant-a")

        result = await tool.execute(
            path=str(tmp_path / "overrides" / "HEARTBEAT.md"),
            content="## Learned Recurring Tasks\n\n- 每2分钟提醒喝水",
        )

        assert "Successfully wrote" in result
        assert document_store.get_active_content("tenant-a", "heartbeat", "HEARTBEAT.md")
        assert tenant_state_repo.calls
        tenant_key, kwargs = tenant_state_repo.calls[-1]
