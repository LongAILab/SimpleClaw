...
import simpleclaw.channels.matrix as matrix_module
from simpleclaw.bus.events import OutboundMessage
from simpleclaw.bus.queue import MessageBus
from simpleclaw.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from simpleclaw.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("simpleclaw.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "simpleclaw.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("simpleclaw.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "simpleclaw.channels.matrix.asyncio.create_task", _fake_create_task
    )
