...
from simpleclaw.providers.registry import find_by_model, find_gateway
...
class LiteLLMProvider(LLMProvider):
    ...
    async def stream_chat(
        ...
        """Stream a chat completion via LiteLLM when supported."""
        original_model = model or self.default_model
        model = self._resolve_model(original_model)
        extra_msg_keys = self._extra_msg_keys(original_model, model)
        ...
        async for chunk in stream:
            ...
            for choice in choices:
                ...
                for tc in delta_tool_calls:
                    ...
                    idx = int(tc.get("index", len(tool_buffers)))
                    ...
                    if not isinstance(function, dict):
                        function = function.model_dump() if hasattr(function, "model_dump") else {}
                    if function.get("name"):
                        buf["name"] = function["name"]
                    if function.get("arguments"):
                        buf["arguments"] += function["arguments"]
                    if tc.get("provider_specific_fields"):
                        buf["provider_specific_fields"] = tc.get("provider_specific_fields")
                    if function.get("provider_specific_fields"):
                        buf["function_provider_specific_fields"] = function.get("provider_specific_fields")
        ...
        for idx in sorted(tool_buffers):
            ...
            tool_calls.append(
                ToolCallRequest(
                    ...
                    function_provider_specific_fields=buf.get("function_provider_specific_fields"),
                )
            )
